...
void appendToCSV(std::string csvPath, const RunStats& stats) {
    ...
    if (!fileExists) {
        file << "Graph Name,Embedding creation time,Subgraph creation time,Ground truth creation time,Training + inference time,Time NNP-NET,Stress,Neighbourhood preservation,Fallback flag,Success,Error\n";
    }
