...
struct RunStats {
    ...
    double np = -1;
    bool fallback = false;
    int subgraphNodeCount = -1;
    double firstIterCoarseningRate = -1.0;
    bool success = true;
...
void appendToCSV(std::string csvPath, const RunStats& stats) {
    ...
    if (!fileExists) {
        file << "Graph Name,Embedding creation time,Subgraph creation time,Ground truth creation time,Training + inference time,Time NNP-NET,Stress,Neighbourhood preservation,Fallback flag,Success,Error,Subgraph node count,First iter coarsening rate\n";
    }
    ...
    if (subTime == 0) subTime = NNPNet::StatsCollector::times["Create subgraph"];
    file << subTime << ",";
    ...
    if (stats.subgraphNodeCount >= 0) {
        file << stats.subgraphNodeCount;
    }
    file << ",";
    if (stats.firstIterCoarseningRate >= 0) {
        file << stats.firstIterCoarseningRate;
    }
    file << "\n";
}
...
std::unordered_map<std::string, Argument> argMap = {
	...
	{"-dsp", ARG_DISABLE_STALL_PROTECTION},
	{"--dsp", ARG_DISABLE_STALL_PROTECTION},
	{"--disable_stall_protection", ARG_DISABLE_STALL_PROTECTION},
};
...
void createLayoutFor(std::string path, std::string outPath, std::unordered_map<Argument, double>& settings) {
	...
	case METHOD_NNPNET:
	{
		...
		nnpnet.disableStallProtection = settings[ARG_DISABLE_STALL_PROTECTION] > 0;

		fs::path inFsPath(path);
		fs::path subPath;
		if (outPath != "") {
			subPath = fs::path(outPath).parent_path() / (inFsPath.stem().string() + "_subgraph_out.vna");
		} else {
			subPath = inFsPath.parent_path() / (inFsPath.stem().string() + "_subgraph_out.vna");
		}
		...
		currentStats.subgraphNodeCount = nnpnet.lastSubgraphNodeCount;
        currentStats.firstIterCoarseningRate = nnpnet.firstIterCoarseningRate;
		}
...
int main(int argc, char* argv[])
{
	...
	settings[ARG_DISABLE_STALL_PROTECTION] = false;
	...
	if (argc == 1) {
		...
		for (; i < argc; i++) {
			...
			case ARG_DISABLE_STALL_PROTECTION:
	...
	std::string csvPath = outPath == "" ? "results.csv" : outPath + "/results.csv";
	if (outPath != "" && outPath[outPath.size() - 1] != '/' && outPath[outPath.size() - 1] != '\\' && path[path.size() - 4] != '.') {
	...
	if (path[path.size() - 4] == '.') {
		...
		for (const auto& entry : fs::directory_iterator(path)) {
            ...
            try {
                ...
                currentStats.success = false;
                currentStats.errorMessage = e.what();
                std::cout << "Error processing " << currentStats.graphName << ": " << e.what() << "\n";
            } catch (...) {
                currentStats.success = false;
...
void init(int smoothing = 3,
	...
	bool disableStallProtection = false) {
	...
	currentSettings[ARG_DISABLE_STALL_PROTECTION] = disableStallProtection;
};
...
void run(std::string path, std::string outPath, int smoothing = 3,
	...
	settings[ARG_DISABLE_STALL_PROTECTION] = disableStallProtection;
...
void runNNPNET(Graph<double>* graph) {
	std::cerr << "[DEBUG] Entered runNNPNET. disableStallProtection=" << currentSettings[ARG_DISABLE_STALL_PROTECTION] << std::endl;
	currentSettings[ARG_METHOD] = METHOD_NNPNET;
...
PYBIND11_MODULE(NNP_NET, m/*, py::mod_gil_not_used()*/) {
	...
	m.def("initialize", &init,
		...
		"disableStallProtection"_a = false);
	m.def("runAll", &run,
		"path"_a,
		"outPath"_a = "",
		"smoothing"_a = 3,
