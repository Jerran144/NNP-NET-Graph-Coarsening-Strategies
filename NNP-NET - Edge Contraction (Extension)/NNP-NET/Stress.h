...
public:
  static double Calc(Graph<float> &g) {
    ...
    unsigned long long size =
        (unsigned long long)(amountToCheck) * (unsigned long long)g.nodeCount;
    std::pair<float, float> *allData = (std::pair<float, float> *)malloc(
        size * sizeof(std::pair<float, float>));
    float *_f_allData = (float *)allData;
    double top = 0;
    double bot = 0;
    auto res = Threadpool::divideWork<
        std::pair<std::pair<float, float>, std::pair<float, float>>>(
        [inc, allData, &g, _f_allData](int begin, int end)
            -> std::pair<std::pair<float, float>, std::pair<float, float>> {
          double top = 0;
          double bot = 0;
          std::pair<float, float> max = {0, 0};
          for (int a = begin; a < end; a++) {
            ...
            for (long long other = g.nodeCount - 1; other >= 0; other--) {
              ...
              allData[(long long)a * (long long)g.nodeCount + (long long)other]
                  .first = _f_allData[((long long)2 * (long long)a *
                                       (long long)g.nodeCount) +
                                      (long long)other];
              std::pair<float, float> dif = {g.Y[i * 2] - g.Y[other * 2],
                                             g.Y[i * 2 + 1] -
                                                 g.Y[other * 2 + 1]};
              allData[(long long)a * (long long)g.nodeCount + (long long)other]
                  .second =
                  sqrt(dif.first * dif.first + dif.second * dif.second);
              if (allData[(long long)a * (long long)g.nodeCount +
                          (long long)other]
                      .first > max.first) {
                max.first = allData[(long long)a * (long long)g.nodeCount +
                                    (long long)other]
                                .first;
              }
              if (allData[(long long)a * (long long)g.nodeCount +
                          (long long)other]
                      .second > max.second) {
                max.second = allData[(long long)a * (long long)g.nodeCount +
                                     (long long)other]
                                 .second;
              }
              if ((double)allData[(long long)a * (long long)g.nodeCount +
                                  (long long)other]
                      .first <= 0) {
                continue;
              }
              double temp =
                  (double)allData[(long long)a * (long long)g.nodeCount +
                                  (long long)other]
                      .second /
                  (double)allData[(long long)a * (long long)g.nodeCount +
                                  (long long)other]
  ...
  static double CalcFeatureStress(Graph<float> &g) {
    ...
    auto res = Threadpool::divideWork<
        ...
        [inc, allData, &g, _f_allData](int begin, int end)
          ...
          for (int a = begin; a < end; a++) {
            long long i = (long long)(a * inc);
            g.addFeaturesToDistances(
                i, 1, (float *)(allData + a * (long long)g.nodeCount));
            for (long long other = g.nodeCount - 1; other >= 0; other--) {
              if (other == a) {
                allData[a * (long long)g.nodeCount + other] = {0, 0};
                continue;
...
private:
  template <class T>
  static double getStress(Graph<T> &g, long long amountToCheck, double alpha,
                          std::pair<float, float> *allData) {
    double stress = 0;
    for (long long i = 0; i < (long long)amountToCheck * (long long)g.nodeCount;
         i++) {
      if (allData[i].first < DISTANCE_THRESHOLD)
        continue;
      std::pair<float, float> stressVec =
          std::pair<float, float>{allData[i].first * alpha, allData[i].second};
